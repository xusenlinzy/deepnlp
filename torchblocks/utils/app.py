...
def visualize_ner(text, rlt, colors, span=False):
    ...
    for r, text in zip(rlt, texts):
        if span:
            spans = [{"start_token": t["start"], "end_token": t["end"], "label": k} for k, v in r.items() for t in v]
            ex = [{"text": text, "spans": spans, "tokens": list(text)}]
            html = displacy.render(ex, style="span", manual=True, options={"colors": colors})
        else:
            ents = [{"start": t["start"], "end": t["end"], "label": k} for k, v in r.items() for t in v]
