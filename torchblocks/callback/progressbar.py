...
class ProgressBar(object):
    ...
    def _time_info(self, now, current):
        ...
        if current < self.n_total:
            ...
            if eta > 3600:
                ...
                eta_format = '%ds' % eta
            return f' - ETA: {eta_format}'
        elif time_per_unit >= 1:
            return f' {time_per_unit:.1f}s/step'
        elif time_per_unit >= 1e-3:
            return f' {time_per_unit * 1e3:.1f}ms/step'
        else:
            return f' {time_per_unit * 1e6:.1f}us/step'

    def _bar(self, current):
        recv_per = current / self.n_total
        bar = f'[{self.desc}] {current}/{self.n_total} ['
        recv_per = min(recv_per, 1)
    ...
    def step(self, step, info=None):
        ...
        if len(info) != 0:
            show_bar = (f'{show_bar}  [' + "-".join([f' {key}={value:4f}  ' for key, value in info.items()])) + "]"
