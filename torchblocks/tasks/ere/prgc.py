        ...
        def decode(self, sequence_output, rel_pred, corres_pred, attention_mask, texts, offset_mapping):
            ...
            for i in range(bs):
                ...
                if len(rels) > 0:
                    ...
                    pred_seq_sub = pred_seq_sub.detach().cpu().numpy()
                    pred_seq_obj = pred_seq_obj.detach().cpu().numpy()
                    pre_corres = pre_corres.detach().cpu().numpy()
                    spoes = self.get_triples(rels, pred_seq_sub, pred_seq_obj, pre_corres, texts[i], offset_mapping[i])
                decode_labels.append(spoes)

            return decode_labels

        def get_triples(self, rels, pred_seq_sub, pred_seq_obj, pre_corres, text, mapping):
            triples = set()
            for i in range(len(rels)):
                seq_sub_tags = [self.config.id2label[t] for t in pred_seq_sub[i]]
                seq_obj_tags = [self.config.id2label[t] for t in pred_seq_obj[i]]
                    ...
                    for t in objects:
                        if pre_corres[h[1]][t[1]] == 1:
                            triples.add((
                                ...
                                self.config.id2predicate[rels[i]],
