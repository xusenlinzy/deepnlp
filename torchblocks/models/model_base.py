...
class TrainModel(nn.Module):
    def __init__(self, config, *inputs, **kwargs):
        ...
        if not isinstance(config, TrainConfig):
            raise ValueError(
                f"Parameter config in `{self.__class__.__name__}(config)` should be an instance of class `TrainConfig`. To create a model from a train model use `model = {self.__class__.__name__}.load(model_path)`")
    ...
    def from_pretrained(cls, model_path, *model_args, **kwargs):
        ...
        if os.path.isdir(model_path):
            if os.path.isfile(os.path.join(model_path, WEIGHTS_NAME)):
                ...
                logger.warning(f"Error no file named {[WEIGHTS_NAME]} found in directory {model_path} ")
